...
module.exports = async function handler(req, res) {
  ...
  if (req.headers.authorization !== `Bearer ${process.env.ADMIN_SECRET}`) return res.status(401).json({ error: 'Unauthorized' });
  ...
  if (req.method === 'POST' && action === 'create') {
    ...
    const { data, error } = await supabase.from('license_keys').insert({ key, name: name||'Chưa đặt tên', expires_at, plan: plan||'basic', active: true, created_at: new Date().toISOString(), bound_ip: null, use_count: 0 }).select().single();
